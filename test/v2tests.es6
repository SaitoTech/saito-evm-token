...
const { signMessageForTesting, makeMintingMessage32, splitSignature, manuallySign } = require('../scripts/lib/helperfunctions');
...
contract('Test Contracts', (accounts) => {
  ...
  describe('SaitoTokenV2 Tests', function() {
    ...
    it("saitoTokenV2 can be minted by owners", async function() {
      ...
      let totalSupply = await saitoTokenV2.totalSupply.call();
      assert.equal(totalSupply.sub(initSupply).toNumber(), 0, "expected supply of " + initSupply);
      ...
      assert.equal(owner1Balance.toNumber(), 0, "expected owner 1 to have 0");
      assert.equal(owner2Balance.toNumber(), 0, "expected owner 2 to have 0");
      assert.equal(owner3Balance.sub(initSupply).toNumber(), 0, "expected owner 3 to have init supply");
    });
    ...
    it("saitoTokenV2 after minting, authorizations are reset", async function() {
      ...
      assert.equal(totalSupply.sub(initSupply).sub(initSupply).toNumber(), 0, "expected supply of " + 2*initSupply);
      let owner1Balance = await saitoTokenV2.balanceOf(owner1);
      let owner2Balance = await saitoTokenV2.balanceOf(owner2);
      let owner3Balance = await saitoTokenV2.balanceOf(owner3);
      assert.equal(owner1Balance.sub(initSupply).toNumber(), 0);
      assert.equal(owner2Balance.toNumber(), 0);
      assert.equal(owner3Balance.sub(initSupply).toNumber(), 0);
    });
    ...
    it("saitoTokenV2 owners can revoke minting auth", async function() {
      ...
      await saitoTokenV2.authorizeMinting(initSupply, {from: owner2});
      totalSupply = await saitoTokenV2.totalSupply.call();
      assert.equal(totalSupply.sub(initSupply).sub(initSupply).sub(initSupply).toNumber(), 0, "expected supply of 3*initSupply");
      let owner1Balance = await saitoTokenV2.balanceOf(owner1);
      let owner2Balance = await saitoTokenV2.balanceOf(owner2);
      let owner3Balance = await saitoTokenV2.balanceOf(owner3);
      assert.equal(owner1Balance.sub(initSupply).toNumber(), 0);
      assert.equal(owner2Balance.sub(initSupply).toNumber(), 0);
      assert.equal(owner3Balance.sub(initSupply).toNumber(), 0);
    });
    ...
    it("saitoTokenV2 can be transferred", async function() {
      ...
      assert.equal(newOwnerBalance.sub(ownerBalance.sub(new BN(1000))).toNumber(), 0);
      assert.equal(newUser1Balance.sub(user1Balance.add(new BN(1000))).toNumber(), 0);
    });
    ...
    it("saitoTokenV2 cannot mint more than 10,000,000,000", async function() {
      let totalSupply = await saitoTokenV2.totalSupply.call();
      await saitoTokenV2.authorizeMinting(maxSupply.sub(totalSupply).add(new BN(1)), {from: owner1}).then(() => {
        throw null;
      }).catch(function(error) {
        assert.isNotNull(error, "Expected unapproved revert");
        assert.oneOf(error.message, ["Error: Revert or exceptional halt", "Returned error: VM Exception while processing transaction: revert"], "Expected unapproved revert")
    ...
    // });
    // 
    // it("saitoTokenV2 can be minted with a single transaction and cannot be replayed", async function() {
    ...
    //   await saitoTokenV2.mint(message, sigObj1[0], sigObj1[1], sigObj1[2], sigObj2[0], sigObj2[1], sigObj2[2], sigObj3[0], sigObj3[1], sigObj3[2], {from: owner1});
    //   let newTotalSupply = await saitoTokenV2.totalSupply.call();
    //   let newOwner1Balance = await saitoTokenV2.balanceOf(owner1);
    //   assert.equal(262144000000, initSupply)
    ...
    //   saitoTokenV2.mint(message, sigObj1[0], sigObj1[1], sigObj1[2], sigObj2[0], sigObj2[1], sigObj2[2], sigObj3[0], sigObj3[1], sigObj3[2], {from: owner1}).then(() => {
    //     throw null;
    //   }).catch(function(error) {
    //     assert.isNotNull(error, "Expected unapproved revert");
    //     assert.oneOf(error.message, ["Returned error: VM Exception while processing transaction: revert", "Error: Revert or exceptional halt"]);
